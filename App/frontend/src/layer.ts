...
/**
 * Adds NVDB road segments as a vector layer (temporary for testing).
 * Fetches data from the database based on the current map bounds.
 * @param map The map to add the layer to
 * @param visible Whether the layer starts visible (default true)
 */
export async function AddNVDBRoadsLayer(map: MaplibreGL.Map, visible: boolean = true): Promise<void> {
  const layerVisibility = visible ? 'visible' : 'none';
  const sourceId = 'nvdb-roads-source';
  const layerId = 'nvdb-roads-layer';

  // Function to convert WGS84 (lat/lon) to Web Mercator (EPSG:3857)
  const lngLatToWebMercator = (lng: number, lat: number): [number, number] => {
    const x = (lng / 180) * 20037508.34;
    const y = Math.log(Math.tan(((lat + 90) / 360) * Math.PI)) * 20037508.34;
    return [x, y];
  };

  try {
    // Function to fetch and update roads based on current map bounds
    const updateRoads = async () => {
      const bounds = map.getBounds();
      const [min_x_3857, min_y_3857] = lngLatToWebMercator(bounds.getWest(), bounds.getSouth());
      const [max_x_3857, max_y_3857] = lngLatToWebMercator(bounds.getEast(), bounds.getNorth());

      try {
        const res = await fetch(`/api/nvdb/roads?min_x=${min_x_3857}&min_y=${min_y_3857}&max_x=${max_x_3857}&max_y=${max_y_3857}`);
        const geojson = await res.json();
        ...
        console.error('Failed to fetch NVDB roads:', err);
      }
    };

    // Initial fetch - get bounds from user location or default
    const bounds = map.getBounds();
    const [min_x_3857, min_y_3857] = lngLatToWebMercator(bounds.getWest(), bounds.getSouth());
    const [max_x_3857, max_y_3857] = lngLatToWebMercator(bounds.getEast(), bounds.getNorth());
    ...
    console.log('NVDB data received:', geojson.features?.length, 'features');
    ...
    registerLayer(layerId, 'NVDB Veinettverk', visible, map);

    // Update roads when map is moved or zoomed (debounced)
    ...
    console.error('Failed to add NVDB roads layer:', err);
  }
}
